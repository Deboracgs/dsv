...
import { userSelector, searchUsers } from "./effects/reducers";
import { Counter, InputText, ListUser } from "./components";
...
export default function App() {
  ...
  const { originalUsers } = useSelector(userSelector);

  const searchUser = (search: string) => {
    let usersSearch = []
    if(search === ""){
    usersSearch = originalUsers.filter((u) => u.deleted === false);
    dispatch(searchUsers({users: usersSearch, onSearch: false}));
    return;
    }
    usersSearch = originalUsers.filter((u) => u.username.toLocaleLowerCase().includes(search.toLocaleLowerCase()))
    dispatch(searchUsers({users: usersSearch, onSearch: true}))
  }

  return (
    <div data-testid="app">
      <Box sx={{ flexGrow: 1 }}>
        <Grid container spacing={2} marginY={2} columns={{ md: 12 }}>
          <Grid item md={12}>
            <Counter />
          </Grid>
        </Grid>
        <Grid container spacing={2} marginY={2} columns={{ md: 12 }}>
          <Grid item md={12}>
            <InputText id="search-user" label="Search for a user" defaultValue={""} onChange={(value) => searchUser(value)}></InputText>
          </Grid>
        </Grid>
        <Grid container spacing={2} marginY={2} columns={{ md: 12 }}>
          <Grid item md={12}>
